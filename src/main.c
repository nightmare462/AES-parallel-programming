...
int main(int argc, char *argv[])
{
    ...
    unsigned char key[16] = {'I', 't', 'i', 's', 's', 'e', 'c', 'r', 'e', 't', 'e', 'k', 'e', 'y', '.', '.'};
    ...
    expandKey(expandedKey, key, KEY_SIZE, expandedKeySize);

    // AES Encryption
    for (size_t i = 0; i < numBlocks; i++) {
        aes_encrypt(inputData + i * BLOCK_SIZE, outputData + i * BLOCK_SIZE, key, KEY_SIZE);
    }

    clock_t End = clock();
    printf("\nTime: %ld",(Start-End)/CLOCKS_PER_SEC);
    ...
    // AES Decryption
    for (size_t i = 0; i < numBlocks; i++) {
        aes_decrypt(outputData + i * BLOCK_SIZE, decryptedData + i * BLOCK_SIZE, key, KEY_SIZE);
    }
