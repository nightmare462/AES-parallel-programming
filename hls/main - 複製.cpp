...
#pragma HLS DATAFLOW
  ...
  for (int i = 0; i < 4; ++i) {
    shiftRow(hs_state_inp, hs_state_out, i);
  }
}
...
void mixColumns(hls::stream<uint8_t> &hs_state_inp,
  ...
  for (int i = 0; i < 4; ++i) {
    for (int j = 0; j < 4; ++j) {
      hs_column_inp[j] << hs_state_inp.read();
    }
  }
...
#pragma HLS UNROLL
  ...
  for (int i = 0; i < 4; ++i) {
    for (int j = 0; j < 4; ++j) {
      hs_state_out << hs_column_out[j].read();
    }
  }
}
...
#pragma HLS INTERFACE mode = axis port = hs_output
  ...
  // Transform the block
  MAIN_READ_INPUT: for (int i = 0; i < 4; ++i) {
    for (int j = 0; j < 4; ++j) {
  ...
  MAIN_WRITE_OUTPUT: for (int i = 0; i < 4; ++i) {
    for (int j = 0; j < 4; ++j) {
      ap_uint8_t tmp;
      tmp.data = hs_block_out_column[i].read();
      tmp.last = (i + j * 4) == 15;
      hs_output.write(tmp);
    }
  }
